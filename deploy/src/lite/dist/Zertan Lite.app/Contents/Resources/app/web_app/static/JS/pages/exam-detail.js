...
export async function initExamDetailPage(pageContext) {
    ...
    const renderLoadingState = () => {
        renderCardSkeletons(header, { count: 1, showBadge: false, chips: 0, actions: 2 });
        renderSelectionFieldSkeleton(filters, { count: 2, includePanels: true });
        renderPaginationSkeleton(paginationTop);
        renderQuestionSkeletons(questionContainer, QUESTIONS_PER_PAGE);
        renderPaginationSkeleton(paginationBottom);
    };
    ...
    const loadExamDetail = async () => {
        ...
        `;
            ...
            function renderQuestions() {
                clearBusyState(questionContainer);
                clearBusyState(paginationTop);
                clearBusyState(paginationBottom);
            ...
            paginationTop.innerHTML = "";
            paginationBottom.innerHTML = "";
            renderErrorState(questionContainer, {
                title: "Unable to load study mode",
                message: error.message,
                onRetry: loadExamDetail,
            });
        }
    };
