...
class DatabaseRuntimeTests(unittest.TestCase):
    ...
    def test_transaction_rolls_back_all_changes_when_an_error_is_raised(self):
        database = self._build_seeded_database()

        with self.assertRaisesRegex(RuntimeError, "forced rollback"):
            with database.transaction():
                database.execute("INSERT INTO tags (name) VALUES (?)", ("transaction-tag",))
                database.execute("INSERT INTO topics (name) VALUES (?)", ("transaction-topic",))
                raise RuntimeError("forced rollback")

        _, tag_row = database.execute(
            "SELECT COUNT(*) AS total FROM tags WHERE name = ?",
            ("transaction-tag",),
            fetchone=True,
        )
        _, topic_row = database.execute(
            "SELECT COUNT(*) AS total FROM topics WHERE name = ?",
            ("transaction-topic",),
            fetchone=True,
        )

        self.assertEqual(tag_row["total"], 0)
        self.assertEqual(topic_row["total"], 0)
