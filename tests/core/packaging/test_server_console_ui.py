...
from deploy.src.server import server_launcher
from deploy.src.server.console_ui.formatting import format_uptime
...
class ApiRequestConsoleLogTests(unittest.TestCase):
    ...
    def test_loader_ignores_top_level_console_ui_asset_namespace(self):
        original_module = sys.modules.get("console_ui")
        sys.modules["console_ui"] = types.ModuleType("console_ui")
        try:
            module = server_launcher.load_server_console_ui_module()
        finally:
            if original_module is None:
                sys.modules.pop("console_ui", None)
            else:
                sys.modules["console_ui"] = original_module

        self.assertTrue(hasattr(module, "ApiRequestConsoleLog"))
