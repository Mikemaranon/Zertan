...
from services_m import LiveExamService
...
class LiveExamsAPI(BaseAPI):
    ...
    def list_live_exams(self):
        ...
        service = LiveExamService(self.db)
        ...
        return self.ok(
            {
                "mode": "user",
                "assignments": service.list_for_user(user["id"]),
                "feature_enabled": self.feature_enabled("live_exams_page"),
            }
        )
    ...
    def create_live_exam(self):
        ...
        service = LiveExamService(self.db)
        try:
            live_exam = service.create_live_exam(request.get_json() or {}, user["id"])
    ...
    def close_live_exam(self, live_exam_id):
        ...
        service = LiveExamService(self.db)
        try:
            live_exam = service.close_live_exam(live_exam_id)
        except ValueError as exc:
            return self.error(str(exc), 400)
    ...
    def delete_live_exam(self, live_exam_id):
        ...
        service = LiveExamService(self.db)
        try:
            service.delete_live_exam(live_exam_id)
        except ValueError as exc:
            return self.error(str(exc), 400)
    ...
    def start_assignment(self, assignment_id):
        ...
        service = LiveExamService(self.db)
        try:
            attempt_id = service.start_assignment(assignment_id, user)
        except ValueError as exc:
            return self.error(str(exc), 400)
