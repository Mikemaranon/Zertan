...
def validate_exam_scope_group_ids(db, values, *, allowed_group_ids=None, allow_global=True):
    ...
    existing_group_ids = set(_groups_repository(db).list_existing_ids(normalized_group_ids))
...
def _groups_repository(db):
    ...
    if groups is None:
        raise TypeError("validate_exam_scope_group_ids requires a DB manager exposing db.groups.")
    return groups
